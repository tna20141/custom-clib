...
static void test_in_out_peek(void **state)
{
	struct bq_queue queue;
	teste e1, e2, e3, e4, *e;
	...
	e = (teste *)bq_out_head(&queue);
	assert_int_equal(NULL, e);
	verify_queue(&queue, 0);
	e = (teste *)bq_out_tail(&queue);
	assert_int_equal(NULL, e);
	verify_queue(&queue, 0);
	e = (teste *)bq_out(&queue);
	assert_int_equal(NULL, e);
	verify_queue(&queue, 0);
	...
	assert_int_equal(BE_OK, bq_in_tail((void *)&e1, &queue));
	verify_queue(&queue, 1, 1);
	assert_int_equal(BE_OK, bq_in((void *)&e2, &queue));
	verify_queue(&queue, 2, 1, 2);
	assert_int_equal(BE_OK, bq_in_head((void *)&e3, &queue));
	verify_queue(&queue, 3, 3, 1, 2);
	bq_in_tail((void *)&e4, &queue);
...
static void test_foreach(void **state)
{
	...
	bq_for_each_head(pos, teste, &queue)
		n++;
	assert_int_equal(0, n);
	bq_for_each_head_safe(pos, tmp, teste, &queue)
		n++;
	assert_int_equal(0, n);
	bq_for_each_tail(pos, teste, &queue)
		n++;
	assert_int_equal(0, n);
	bq_for_each_tail_safe(pos, tmp, teste, &queue)
		n++;
	assert_int_equal(0, n);
	bq_for_each(pos, teste, &queue)
		n++;
	assert_int_equal(0, n);
	bq_for_each_safe(pos, tmp, teste, &queue)
		n++;
	assert_int_equal(0, n);
